 ...
 */
function createVertex(parent, coordinates, path, selected) {
  var vertex = {
    type: geojsonTypes.FEATURE,
    properties: Object.assign({}, parent.properties,
      {meta: meta.VERTEX,
...
function createSupplementaryPoints(geojson, options, basePath) {
  ...
  function processLine(line, lineBasePath) {
    ...
    line.forEach(function (point, pointIndex) {
      ...
      var vertex = createVertex(geojson, point, pointPath, isSelectedPath(pointPath));

      // If we're creating midpoints, check if there was a
      // vertex before this one. If so, add a midpoint
      // between that vertex and this one.
      if (options.midpoints && lastVertex) {
        var midpoint = createMidpoint(geojson, lastVertex, vertex);
        if (midpoint) {
